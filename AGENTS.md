...
1. 作業開始時はGit状態・適用指示・変更対象を確認し、検索結果・diff・関連箇所から読み始めます。必要になった依存先・設計文書・履歴へ段階的に広げ、未変更ファイルを理由なく全文再読しません。コード・テスト・履歴から判断できる軽微な不明点は既存仕様を保つ最小変更で進めます。
2. 原則として `main` を直接変更せず、目的が分かる専用branchを作ります。例: `fix/v012-av-sync`、`feat/v020-motion-director`。
3. 変更範囲を必要最小限に限定し、ユーザーから明示されていない補間ロジック、色管理、A/V同期、モデル選択、出力互換性を勝手に変更しません。ただし、コード・テスト・設計文書で確立済みの挙動へ戻す最小限のバグ修正は、新しい製品仕様変更として扱いません。
4. 不具合修正では症状だけでなく根本原因を特定し、可能なら修正前に再現テストまたは回帰テストを追加します。Mock、fixture、CPU代替経路と実CUDA/RIFE実装の仕様差が原因なら、テスト側も実仕様に合わせます。
5. 既存テストを削除・弱体化せず、変更に対応するテストを追加します。作業中は直接関係するテストから関連テストへ広げ、ランタイム変更のPR完成前は標準ゲートを確認します。同一差分で信頼できるCIが成功済みなら理由なく重複実行しません。
6. 変更差分を確認し、`VideoFrame` / `FrameFormat`、PTS、scene cut、色タグ/range/SAR/bit depth、音声・字幕保持、モデルハッシュ、OOM回復、リソース解放への副作用がないかレビューします。
7. 修正branchにコミットし、`main` 向けPull Requestを作ります。PR本文は変更理由・変更ファイル・検証結果・実機未測定項目を簡潔にまとめ、長い成功ログや同じ情報を繰り返しません。
8. CIとレビュー結果を確認します。CUDA/GPU、NVENC、VRAM、実動画A/V同期、画質・性能など実機依存項目は、実測前に「検証済み」と扱いません。
9. ランタイムに影響する変更は、変更に対応するWindows 11/GPU/実動画等の実機確認が完了して問題がなければ「マージ可能」と報告します。PR作成やCI成功はマージ許可を意味せず、ユーザーの明示的な依頼がある場合だけマージします。問題があれば同じbranch/PRで最小限の修正を続け、文書のみの変更はCIと差分レビューを基準にします。

ZIPやチャット添付ファイルが提供された場合も、GitHub版との対応関係を確認し、可能ならGitHubを正本として差分管理します。GitHubへ反映できない場合だけ一時的にZIPベースで作業し、その状態を明記します。

必要な根拠が揃った後は「念のため」だけで検索・全文読込・テストを続けません。AGENTS.mdやSkillの重複・矛盾・古い指示を見つけた場合も、毎回ルールを追加せず、安全に整理できるときだけ既存文面へ統合します。

次のいずれかを観測したら、その判断が必要な箇所で停止して報告します: 同じ根本原因仮説・同等の修正を2回試して同じ失敗が残り新しい証拠がない、依頼の前提がリポジトリの証拠と矛盾し前提変更なしでは達成不能、ユーザー指定の制約同士が両立不能、達成に未承認の補間思想・色管理・A/V同期・モデル・出力互換性の変更または依頼範囲外への拡大が必要。

範囲外で、未確認モデルコードの実行、入力・出力データの破損、秘密情報の漏洩、Original Motion Preservation・A/V同期・色管理等の検証結果を無効にする重大な問題を見つけた場合は勝手に直さず報告します。それ以外の通常の障害は、依頼範囲と既存権限で可能な明白な解決を試す前にエスカレーションしません。
...
- 不具合修正では原因を再現する回帰テストを確認し、直接関連テスト→関連テスト→必要な標準ゲートの順で検証します。文書のみの変更はdiff・リンク・manifest等の対象確認を優先し、形式的なランタイム回帰を追加しません。通過させる目的で既存の検査を削除・弱体化しません。
